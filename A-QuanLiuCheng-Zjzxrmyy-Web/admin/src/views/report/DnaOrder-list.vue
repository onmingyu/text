...
import { GetDnaOrderList, PrintDnaOrder, ChannelBillListExcel, base } from "../../api/api";
...
export default {
  ...
  methods: {
    ...
    getExcel(){
			// window.open(base + '/DnaOrder/ChannelBillListExcel?startTime=' + this.startDate + '&endTime=' + this.endDate)
      var that = this
      that.exportlist = []
      let para = {
        startDate: this.startDate,
      ...
      this.listLoading = true;
      ChannelBillListExcel(para).then(res => {
        ...
        that.exportExcelData('核酸检测登记表明细', that.exportlist)
      });
    },
