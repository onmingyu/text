...
Page({
  data: {
    ...
    webhomeurl: 'http://localhost:8081/#!/index/portal',               // 首面   
    ...
    // webhomeurl: 'http://192.168.4.70:8081/#!/index/portal',       // + '?myUserId=oxKlnuJnTV6TgnZ7hqzG_0-spINI'
    ispay: false, //是否支付
    isAuth: false,
  ...
  onWebMessage(e) {
    ...
    switch (detail.msgType) {
      ...
      case 'check':
        ...
        break
      case 'patientCard':
        self.getAuthorizations("patientCard")
        break
  ...
  getAuthorizations(msgType){
    var self = this
    my.getAuthCode({
          scopes: ['auth_user','mfrstre'],
          success: (res) => {
            self.webViewContext.postMessage({ msgType: msgType, code: "0", msg: JSON.stringify(res) });
            self.setData({
            isAuth: false
          })
          },
          fail: (res) => {
            self.webViewContext.postMessage({ msgType: msgType, code: "1", msg: JSON.stringify(res) });
            self.setData({
              isAuth: true
            })
          }
