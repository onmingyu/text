      ...
      <div class="weui_msg">
        ...
        <div class="weui_text_area">
          ...
          <p class="weui_msg_desc">
            请牢记您的就诊时间:{{ regDt }} {{ displayRegTime }}, 就诊医生:{{\deptName}}
            {{ drName }}
          </p>
        </div>
...
export default {
  ...
  methods: {
    ...
    doReg: function () {
      ...
      self.setRegOrderNum(self.OrderId, self.OrderNo)
      self.callPay(self.PayString)
    },
    gotoPortal: function () {
    ...
    callPay: function (payString) {
      ...
      if (process.env.PAY_TYPE === 'weixin') {
        ...
        if (navigator.userAgent.indexOf('AlipayClient') > -1) {
          window.my.postMessage({ msgType: 'doregpay', tradeNO: payString })
        } else {
          // 支付操作
          var form = $(payString)
          form.submit()
  ...
  ready: function () {
    ...
    if (navigator.userAgent.indexOf('AlipayClient') > -1) {
      window.my.onMessage = function (e) {
        // {'msgType': 'regpay','code': '0'}  // {'msgType': 'regpay','code': '0'}
        if (e.msgType.toLocaleLowerCase() === 'doregpay') {
          if (e.code === '0') {
            self.paySucceed()
          } else {
            self.payFailed()
    ...
    // }
    if (this.$route.query.urlId) {
      this.$ajax.get('CreateApptOrderPay', 'Reg', { urlId: this.$route.query.urlId }).then(function (data) {
        if (data.ResultCode !== '0') {
          ...
          self.cardNum = data.CardNo
          self.cardName = data.CardName
          self.regDt = data.RegDate
          self.cardSumFee = data.RegFee
          self.displayRegTime = data.displayRegTime
          self.PayString = data.PayString
