      ...
      <div class="weui_cells weui_cells_access">
        <a @click="goOrderDetail(order.id)" class="weui_cell" href="javascript:;">
          ...
          <div class="weui_cell_bd weui_cell_primary">
            ...
            <p>订单号：<span class="am-ft-sm am-ft-blue">{{order.num}}</span></p>
            <p>开单科室：<span class="am-ft-sm am-ft-blue">{{order.orderDept}}</span></p>
            <p>开单医生：<span class="am-ft-sm am-ft-blue">{{order.orderDate}}</span></p>
          </div>
          <div class="weui_cell_bd">
            <p>
              <span class="blue_tag">{{order.prescMoney}} 元</span>
            </p>
          </div>
...
export default {
  ...
  methods: {
    getOrders: function () {
      ...
      this.$ajax.get('GetOrderList', 'BillOrder', { type: this.tabName }).then(function (data) {
        ...
        self.orders = _.map(data.Orders, function (val) {
          return {
            ...
            prescMoney: val.PayMoney
          }
        })
      })
    },
    goOrderDetail: function (orderId) {
      this.$router.go({ name: 'billOrderDetail', params: { orderId: orderId } })
    }
  },
  ready: function () {
    this.getOrders()
