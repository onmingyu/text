    ...
    <div class="bill-detail" v-show="showDetail">
      ...
      <div v-show="yiBaoStatus == 1">
        ...
        <p style="padding: 10px" class="am-ft-sm am-ft-gray">
          ...
          如想去窗口缴费，请点击【取消缴费】按钮后再去窗口缴费。
        </p>
      </div>
      <div>
        <p style="padding: 10px" class="am-ft-sm am-ft-gray">
          <span class="am-ft-orange"><i class="fa fa-info-circle"></i></span>
          {{ platform }}缴费支持自费，不支持公费缴费。
        </p>
      </div>
          ...
          >
            <p>
              <span style="display: block"
                >处方单号：<span style="color: #0ae">{{
                  b.ClinicNo
                }}</span></span
              >
              <span style="display: block"
                >开单时间：<span style="color: #0ae">{{
                  b.OrderDate
                }}</span></span
              >
            </p>
        ...
        <div v-for="item in b.Items" class="weui_cell">
          <div class="weui_cell_bd weui_cell_primary">
            <p>
              ...
              <span style="display: block"
                >名称： <span style="color: #0ae">{{ item.Name }}</span></span
              >
              <span style="display: block; width: 100%">
                ...
                >
                <span style="display: inline-block; width: 50%; float: left"
                  >单位：<span style="color: #0ae">{{ item.Unit }}</span></span
                >
              </span>
              <span style="display: block; width: 100%">
                <span style="display: inline-block; width: 50%; float: left"
                  >单价：<span style="color: #0ae"
                    >{{ item.ItemPrice }} 元</span
                  ></span
                >
                <span style="display: inline-block; width: 50%; float: left"
    ...
    callPay: function (payString) {
      ...
      if (process.env.PAY_TYPE === 'weixin') {
        ...
        if (navigator.userAgent.indexOf('AlipayClient') > -1) {
          window.my.postMessage({ msgType: 'billpay', tradeNO: payString })
        } else {
          // 支付操作
          var form = $(payString)
          form.submit()
  ...
  ready: function () {
    ...
    if (navigator.userAgent.indexOf('AlipayClient') > -1) {
      window.my.onMessage = function (e) {
        // {'msgType': 'billpay','code': '0'}  // {'msgType': 'billpay','code': '0'}
        if (e.msgType.toLocaleLowerCase() === 'billpay') {
          if (e.code === '0') {
            self.paySucceed()
          } else {
            self.payFailed()
    ...
    this.$ajax.get('CreateBillOrderNew', 'Bill', {
      ...
      if (data.ResultCode !== '0') {
        ...
        //     items: _.map(val.Items, function (val) {
        //       return {
        //         type: val.Type,
        //         name: val.Name,
        //         account: val.Account,
