      ...
      <div class="weui_cells weui_cells_access">
        <a @click="goOrderDetail(order.id)" class="weui_cell" href="javascript:;">
          ...
          <div class="weui_cell_bd weui_cell_primary">
            <p><span class="am-ft-12 am-ft-gray">{{order.cardNo}}</span></p>
            <p>登记时间：<span class="am-ft-sm am-ft-blue">{{order.createDt}}</span></p>
          </div>
...
export default {
  ...
  methods: {
    getOrders: function () {
      ...
      this.$ajax.get('GetOrderList', 'Dna', { type: this.tabName }).then(function (data) {
        ...
        self.orders = _.map(data.Orders, function (val) {
          return {
            ...
            RegisTerDate: val.RegisTerTime,
            SampDate: val.SampTime,
            sumFee: val.SumFee,
    ...
    goOrderDetail: function (orderId) {
      this.$router.go({ name: 'doDnaOrderDetail', query: { orderId: orderId } })
    }
  },
  ready: function () {
    this.getOrders()
